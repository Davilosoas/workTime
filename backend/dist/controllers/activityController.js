"use strict";
var __awaiter =
  (this && this.__awaiter) ||
  function (thisArg, _arguments, P, generator) {
    function adopt(value) {
      return value instanceof P
        ? value
        : new P(function (resolve) {
            resolve(value);
          });
    }
    return new (P || (P = Promise))(function (resolve, reject) {
      function fulfilled(value) {
        try {
          step(generator.next(value));
        } catch (e) {
          reject(e);
        }
      }
      function rejected(value) {
        try {
          step(generator["throw"](value));
        } catch (e) {
          reject(e);
        }
      }
      function step(result) {
        result.done
          ? resolve(result.value)
          : adopt(result.value).then(fulfilled, rejected);
      }
      step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
  };
var __generator =
  (this && this.__generator) ||
  function (thisArg, body) {
    var _ = {
        label: 0,
        sent: function () {
          if (t[0] & 1) throw t[1];
          return t[1];
        },
        trys: [],
        ops: [],
      },
      f,
      y,
      t,
      g = Object.create(
        (typeof Iterator === "function" ? Iterator : Object).prototype
      );
    return (
      (g.next = verb(0)),
      (g["throw"] = verb(1)),
      (g["return"] = verb(2)),
      typeof Symbol === "function" &&
        (g[Symbol.iterator] = function () {
          return this;
        }),
      g
    );
    function verb(n) {
      return function (v) {
        return step([n, v]);
      };
    }
    function step(op) {
      if (f) throw new TypeError("Generator is already executing.");
      while ((g && ((g = 0), op[0] && (_ = 0)), _))
        try {
          if (
            ((f = 1),
            y &&
              (t =
                op[0] & 2
                  ? y["return"]
                  : op[0]
                  ? y["throw"] || ((t = y["return"]) && t.call(y), 0)
                  : y.next) &&
              !(t = t.call(y, op[1])).done)
          )
            return t;
          if (((y = 0), t)) op = [op[0] & 2, t.value];
          switch (op[0]) {
            case 0:
            case 1:
              t = op;
              break;
            case 4:
              _.label++;
              return { value: op[1], done: false };
            case 5:
              _.label++;
              y = op[1];
              op = [0];
              continue;
            case 7:
              op = _.ops.pop();
              _.trys.pop();
              continue;
            default:
              if (
                !((t = _.trys), (t = t.length > 0 && t[t.length - 1])) &&
                (op[0] === 6 || op[0] === 2)
              ) {
                _ = 0;
                continue;
              }
              if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) {
                _.label = op[1];
                break;
              }
              if (op[0] === 6 && _.label < t[1]) {
                _.label = t[1];
                t = op;
                break;
              }
              if (t && _.label < t[2]) {
                _.label = t[2];
                _.ops.push(op);
                break;
              }
              if (t[2]) _.ops.pop();
              _.trys.pop();
              continue;
          }
          op = body.call(thisArg, _);
        } catch (e) {
          op = [6, e];
          y = 0;
        } finally {
          f = t = 0;
        }
      if (op[0] & 5) throw op[1];
      return { value: op[0] ? op[1] : void 0, done: true };
    }
  };
var __importDefault =
  (this && this.__importDefault) ||
  function (mod) {
    return mod && mod.__esModule ? mod : { default: mod };
  };
Object.defineProperty(exports, "__esModule", { value: true });
exports.getActivityReport =
  exports.deleteActivity =
  exports.getActivities =
  exports.createActivity =
    void 0;
var Activity_1 = __importDefault(require("../models/Activity"));
var createActivity = function (req, res) {
  return __awaiter(void 0, void 0, void 0, function () {
    var _a, name_1, startTime, endTime, newActivity, error_1;
    return __generator(this, function (_b) {
      switch (_b.label) {
        case 0:
          _b.label = 1;
        case 1:
          _b.trys.push([1, 3, , 4]);
          (_a = req.body),
            (name_1 = _a.name),
            (startTime = _a.startTime),
            (endTime = _a.endTime);
          if (!name_1 || !startTime || !endTime) {
            return [
              2 /*return*/,
              res
                .status(400)
                .json({ message: "Todos os campos são obrigatórios" }),
            ];
          }
          return [
            4 /*yield*/,
            Activity_1.default.create({
              userId: req.user.userId,
              name: name_1,
              startTime: startTime,
              endTime: endTime,
            }),
          ];
        case 2:
          newActivity = _b.sent();
          res.status(201).json(newActivity);
          return [3 /*break*/, 4];
        case 3:
          error_1 = _b.sent();
          res
            .status(500)
            .json({ message: "Erro ao criar atividade", error: error_1 });
          return [3 /*break*/, 4];
        case 4:
          return [2 /*return*/];
      }
    });
  });
};
exports.createActivity = createActivity;
var getActivities = function (req, res) {
  return __awaiter(void 0, void 0, void 0, function () {
    var activities, error_2;
    return __generator(this, function (_a) {
      switch (_a.label) {
        case 0:
          _a.trys.push([0, 2, , 3]);
          return [
            4 /*yield*/,
            Activity_1.default.findAll({
              where: { userId: req.user.id },
              order: [["startTime", "ASC"]],
            }),
          ];
        case 1:
          activities = _a.sent();
          res.status(200).json(activities);
          return [3 /*break*/, 3];
        case 2:
          error_2 = _a.sent();
          res
            .status(500)
            .json({ message: "Erro ao buscar atividades", error: error_2 });
          return [3 /*break*/, 3];
        case 3:
          return [2 /*return*/];
      }
    });
  });
};
exports.getActivities = getActivities;
var deleteActivity = function (req, res) {
  return __awaiter(void 0, void 0, void 0, function () {
    var id, activity, error_3;
    return __generator(this, function (_a) {
      switch (_a.label) {
        case 0:
          _a.trys.push([0, 3, , 4]);
          id = req.params.id;
          return [
            4 /*yield*/,
            Activity_1.default.findOne({
              where: { id: id, userId: req.user.id },
            }),
          ];
        case 1:
          activity = _a.sent();
          if (!activity) {
            return [
              2 /*return*/,
              res.status(404).json({ message: "Atividade não encontrada" }),
            ];
          }
          return [4 /*yield*/, activity.destroy()];
        case 2:
          _a.sent();
          res.status(200).json({ message: "Atividade deletada com sucesso" });
          return [3 /*break*/, 4];
        case 3:
          error_3 = _a.sent();
          res
            .status(500)
            .json({ message: "Erro ao deletar atividade", error: error_3 });
          return [3 /*break*/, 4];
        case 4:
          return [2 /*return*/];
      }
    });
  });
};
exports.deleteActivity = deleteActivity;
var getActivityReport = function (req, res) {
  return __awaiter(void 0, void 0, void 0, function () {
    var activities, report_1, error_4;
    return __generator(this, function (_a) {
      switch (_a.label) {
        case 0:
          _a.trys.push([0, 2, , 3]);
          return [
            4 /*yield*/,
            Activity_1.default.findAll({
              where: { userId: req.user.id },
            }),
          ];
        case 1:
          activities = _a.sent();
          report_1 = {};
          activities.forEach(function (activity) {
            var date = new Date(activity.startTime).toISOString().split("T")[0];
            var duration =
              (new Date(activity.endTime).getTime() -
                new Date(activity.startTime).getTime()) /
              (1000 * 60 * 60);
            report_1[date] = (report_1[date] || 0) + duration;
          });
          res.status(200).json(report_1);
          return [3 /*break*/, 3];
        case 2:
          error_4 = _a.sent();
          res
            .status(500)
            .json({ message: "Erro ao gerar relatório", error: error_4 });
          return [3 /*break*/, 3];
        case 3:
          return [2 /*return*/];
      }
    });
  });
};
exports.getActivityReport = getActivityReport;
